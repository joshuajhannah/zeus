...
TEST(ErrorMessageTest, BadSubscriptErrorMessage) {
  ...
  EXPECT_THROW_EXCEPTION(zeus::yaml::BadSubscript, doc["first"]["second"]["fourth"],
                         "operator[] call on a scalar (key: \"fourth\")");
  
  EXPECT_THROW_EXCEPTION(zeus::yaml::BadSubscript, doc["first"]["second"][37],
                         "operator[] call on a scalar (key: \"37\")");


  // Non-printable key is not included in error message
  EXPECT_THROW_EXCEPTION(zeus::yaml::BadSubscript,
                         doc["first"]["second"][std::vector<int>()],
                         "operator[] call on a scalar");

  EXPECT_THROW_EXCEPTION(zeus::yaml::BadSubscript, doc["first"]["second"][Node()],
                         "operator[] call on a scalar");
}
...
TEST(ErrorMessageTest, Ex9_1_InvalidNodeErrorMessage) {
  ...
  // Test that printable key is part of error message
  EXPECT_THROW_EXCEPTION(zeus::yaml::InvalidNode, doc["first"]["fourth"].as<int>(),
                         "invalid node; first invalid key: \"fourth\"");
  
  EXPECT_THROW_EXCEPTION(zeus::yaml::InvalidNode, doc["first"][37].as<int>(),
                         "invalid node; first invalid key: \"37\"");
 
  // Non-printable key is not included in error message
  EXPECT_THROW_EXCEPTION(zeus::yaml::InvalidNode,
